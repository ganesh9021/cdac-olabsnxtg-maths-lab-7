...
function DragnDrop_6() {
  let navigate = useNavigate();

  const Next_Res = () => {
    navigate("/activity1/letusverify/act1startpage/lhs_rhs/tool_6/dragndrop_6/res_6");
  };
  const canvas = useRef();
  ...
  const boxes = [
    ...
    { x: 75, y: 100, r: 50, s: 0, e: 2 * Math.PI, shapeName: "A" },
    { x: 200, y: 100, r: 50, s: 0, e: 2 * Math.PI, shapeName: "B" },
    { x: 325, y: 100, r: 50, s: 0, e: 2 * Math.PI, shapeName: "C" },
  ];
  let isDown = false;
  let dragTarget = null;
  let startX = null;
